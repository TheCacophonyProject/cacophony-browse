...
describe("query() calls fetch", () => {
  ...
  test("with correctly formatted query string", async () => {
    ...
    expect(parsedString.testQueryNumberValue).toBe(
      params.testQueryNumberValue.toString()
    );
    expect(parsedString.testQueryStringValue).toBe(
      params.testQueryStringValue.toString()
    );
